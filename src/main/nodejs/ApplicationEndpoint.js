...
var ApplicationEndpoint_healthPoke_args = function(args) {
  this.request = null;
  if (args) {
...
var ApplicationEndpoint_healthPoke_result = function(args) {
  this.success = null;
  this.ex1 = null;
...
var ApplicationEndpointClient = exports.Client = function(output, pClass) {
    this.output = output;
    this.pClass = pClass;
    this._seqid = 0;
    this._reqs = {};
...
ApplicationEndpointClient.prototype.send_healthPoke = function(request) {
  ...
  var args = new ApplicationEndpoint_healthPoke_args(params);
  args.write(output);
...
var ApplicationEndpointProcessor = exports.Processor = function(handler) {
...
ApplicationEndpointProcessor.prototype.process_healthPoke = function(seqid, input, output) {
  ...
  if (this._handler.healthPoke.length === 1) {
    Q.fcall(this._handler.healthPoke.bind(this._handler), args.request)
      .then(function(result) {
        var result_obj = new ApplicationEndpoint_healthPoke_result({success: result});
        ...
        if (err instanceof Exceptions_ttypes.OperationFailedException) {
          result = new ApplicationEndpoint_healthPoke_result(err);
          output.writeMessageBegin("healthPoke", Thrift.MessageType.REPLY, seqid);
        } else {
    ...
    this._handler.healthPoke(args.request, function (err, result) {
      ...
      if ((err === null || typeof err === 'undefined') || err instanceof Exceptions_ttypes.OperationFailedException) {
        result_obj = new ApplicationEndpoint_healthPoke_result((err !== null || typeof err === 'undefined') ? err : {success: result});
        output.writeMessageBegin("healthPoke", Thrift.MessageType.REPLY, seqid);
      } else {
